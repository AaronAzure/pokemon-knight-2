Shader "Game2DWaterKit/Built-in Render Pipeline/VertexLit (Supports Lightmaps)/Water"
{
	Properties {
		...
		Pass
		{
			...
			half4 frag (Varyings i) : SV_Target
			{
				...
				half4 c = Water2D_Frag(i);
					...
					c.rgb *= i.lightColor + _WaterEmissionColor * _WaterEmissionColorIntensity;
				#else
					c.rgb *= i.lightColor;
				#endif
	...
	Pass
		{
			...
			half4 frag (Varyings i) : SV_Target
			{
				...
				//Applying light
				#if defined(Water2D_ApplyEmissionColor)
					c.rgb *= (i.lightColor + DecodeLightmap(UNITY_SAMPLE_TEX2D(unity_Lightmap, i.lightmapCoord))) + _WaterEmissionColor * _WaterEmissionColorIntensity;
				#else
