...
public class PlayerControls : MonoBehaviour
{
	...
	void Update()
	{
		...
		{
			if (NO() || player.GetAxis("Move Vertical") < -0.8f)
				LeaveBench();
		}
	...
	void JumpMechanic()
	{
		...
		else
		{
			...
			// Holding jump button
			else if (jumping && player.GetButton("B"))
			{
	...
	{
		Debug.Log("<color=cyan>Midair Jump</color>");
		anim.SetBool("isGrounded", false);
		anim.SetBool("isFalling", false);
		anim.SetTrigger("jump");
		anim.speed = 1;
