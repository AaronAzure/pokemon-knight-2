    ...
    public class TilesetBrush : ScriptableObject, IBrush
    {
        ...
        public uint RefreshLinkedBrush(STETilemap tilemap, int gridX, int gridY, uint tileData)
        {
            ...
            if (brush)
            {
                ...
                tileData = ApplyAndMergeTileFlags(brush.Refresh(tilemap, gridX, gridY, tileData & ~Tileset.k_TileDataMask_Flags), tileData);
        ...
        public virtual Vector2[] GetAnimUVWithFlags(float innerPadding = 0f, int index = 0, uint flags = 0)
        {
            ...
            uint tileData = ApplyAndMergeTileFlags(GetAnimTileData(), flags);            
