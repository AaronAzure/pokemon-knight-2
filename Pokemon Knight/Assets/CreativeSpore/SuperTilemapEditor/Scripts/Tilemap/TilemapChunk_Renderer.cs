...
namespace CreativeSpore.SuperTilemapEditor
{
    public partial class TilemapChunk
    {
        ...
        public static void RegisterAnimatedBrush(IBrush brush, int subTileIdx = -1, byte flags = 0)
        {
            if (s_currUpdatedTilechunk)
            {
                if (subTileIdx >= 0)
                    s_currUpdatedTilechunk.m_animatedTiles.Add(new AnimTileData() { VertexIdx = s_currUVVertex + (subTileIdx << 2), Brush = brush, SubTileIdx = subTileIdx, flags = flags });
                else
                    s_currUpdatedTilechunk.m_animatedTiles.Add(new AnimTileData() { VertexIdx = s_currUVVertex, Brush = brush, SubTileIdx = subTileIdx, flags = flags });
            }
        }
            ...
            for (int ty = 0, tileIdx = 0; ty < m_height; ++ty)
            {
                for (int tx = 0; tx < m_width; ++tx, ++tileIdx)
                {
                    ...
                    if (tileData != Tileset.k_TileData_Empty)
                    {
                        ...
                        {
                            m_animatedTiles.Add(new AnimTileData() { VertexIdx = s_vertices.Count, Brush = tileBrush, SubTileIdx = -1, flags = (byte)(tileData >> 28) });
                        }
